...
#version1
    ...
    def _create_indicator_dropdown(self, indicators, initial_index):
        dropdown = widgets.Dropdown(options=indicators, value=indicators[initial_index],description='Country:')
        dropdown.observe(self._on_change, names=['value'])
        return dropdown
    
    def _create_year_slider(self, min_year, max_year):
        year_slider_label = widgets.Label('Year range: ')
        year_slider = widgets.IntRangeSlider(value=[min_year, max_year],
    ...
    def _create_plot(self, x_indicator, year_range):
        ...
        ax1.set_ylabel("2005 $ PPP", size=16)
        plt.xlim([year_range[0], year_range[-1]])
        plt.title(x_indicator,size=20)
...
IncomeDeciles = IncomeDeciles.create()
...
#version2
    ...
    def __init__(self,n):
        ...
        self._num_dropdown = self._create_indicator_dropdown(numbers, n-1) 
...
#version2
    ...
    def __init__(self, df,n):
        ...
        self.container = widgets.VBox([
            widgets.HTML(
                (
                    """<h1>Compare the Rich/Poor ratio in multiple countries</h1>
...
"""          
    ...
    def _create_indicator_dropdown(self, indicators, initial_index, n):
        ...
        for x in range (0,n):
            s = 'Country ' + str(x+1) + ':'
            if x < 2:
                dropdown.append(widgets.Dropdown(options=indicators, value=indicators[initial_index-x],description=s))
                dropdown[x].observe(self._on_change, names=['value'])
            else:
                dropdown.append(widgets.Dropdown(options=indicators, value=indicators[initial_index-4*x],description=s))
                dropdown[x].observe(self._on_change, names=['value'])
        return dropdown      
...
MultIncomeDeciles = MultIncomeDeciles.create(counter1._num)
...
#version2
    ...
    def __init__(self, df):
        self._df = df
        available_indicators = self._df['Country'].unique()
        available_metrics = ['Your income ratio', 'Comparison to Rich/Poor ratio','Comparison to Gini']
        ...
        self._a_dropdown = self._create_indicator_dropdown(available_deciles, -2)
        self._b_dropdown = self._create_indicator_dropdown(available_deciles, 1)
        self._plot_container = widgets.Output()
        self._year_slider, year_slider_box = self._create_year_slider(
            min(df['Year']), max(df['Year'])
        )
...
"""          
    ...
    def _create_plot(self, a_indicator, b_indicator, x_indicator, y_indicator, year_range):
        ...
        elif y_indicator =='Comparison to Rich/Poor ratio':
            plt.figure(figsize=(10, 7))
            x1 = df[df['Country'] == x_indicator]['Decile 10 Income']/df[df['Country'] == x_indicator]['Decile 1 Income']
            xs = df[df['Country'] == x_indicator][a_indicator]/df[df['Country'] == x_indicator][b_indicator]
            plt.xlabel("Year", size=16)
            plt.ylabel(y_indicator, size=16)
            plt.plot(years, x1, label = "Rich/Poor")
            plt.plot(years, xs, label = "Your Measure")
...
YourMeasure = YourMeasure.create()
...
CountryLCGini = CountryLCGini.create()
...
CountryGinis = CountryGinis.create(count5._num)
